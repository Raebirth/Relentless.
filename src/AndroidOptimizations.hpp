...
//
// BUG FIX (crash-before-open):
//   • Removed spurious `double timestamp` parameter. GD 2.2081's CCEGLView
//     touch methods take exactly (int, int[], float[], float[]). The wrong
//     signature caused Geode to install the hook at a garbage address → crash.
//   • Renamed handleTouchesCancel → handleTouchesCancelled (Cocos2d-x name).
//     Wrong name = method not found by Geode → hook install failure → crash.
//   • Replaced GEODE_INLINE with plain inline (GEODE_INLINE does not exist).
// ============================================================================
class $modify(FastEGLView, CCEGLView) {
    ...
    // Correct Cocos2d-x name: handleTouchesCancelled (with 'd')
